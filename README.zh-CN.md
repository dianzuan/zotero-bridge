...
`/zotron:setup` 会先 ping 一下 bridge；连不上就把插件自带的 `claude-plugin/xpi/zotron.xpi` 复制到你的真实 Downloads 文件夹（自动识别 `E:\Downloads` 之类盘符重定向、OneDrive 转移和 POSIX 默认路径），然后带你走 Zotero 原生的 **工具 → 插件 → ⚙ → 从文件安装附加组件 → 重启**。装完直接对 Claude 说人话——*"找一下我库里关于注意力机制的论文"*、*"把 DOI 10.1038/nature12373 加到 ML 集合里"*、*"把 10、13、16 号条目按 GB/T 7714 格式导出"*。Claude 自动路由到对应子工作流（search / manage / export / OCR / RAG），子工作流调 RPC。

### 路径 B —— Codex / Code CLI

Codex 不需要走 Claude 插件市场。先安装 Python CLI，再在 Zotero 里安装 XPI，并确保 Code CLI 会话能在 `PATH` 上找到 `zotron*` 命令：

```bash
uv tool install "git+https://github.com/dianzuan/zotron.git#subdirectory=claude-plugin/python"
# Zotero 桌面端：工具 → 插件 → ⚙ → 从文件安装附加组件 → claude-plugin/xpi/zotron.xpi
zotron ping
zotron-rag hits "数字经济 识别策略" --collection "Reading" --output jsonl
```

后续 Codex 插件目录预计可复制到 `codex-plugin/`（`.codex-plugin/plugin.json`、`skills/zotero/`、`agents/`、`bin/zotron*`）。在该发布面稳定前，使用上面的 Python CLI + 手动 XPI 路径；RPC 与 RAG/OCR 契约保持一致。
